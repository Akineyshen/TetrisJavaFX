...
public class Model {
    ...
    public Model(int width, int height) {
        this.width = width;
        this.height = height;
        this.board = new Color[height][width]; // Array for storing cell colors
        this.random = new Random();

        resetBoard(); // Reset the game board
        nextPiece = generateRandomPiece();  // Spawn the first piece
        spawnPiece(); // Spawn the first piece
    }

    // Shape object (includes shape and color)
    ...
    private Piece generateRandomPiece() {
        int pieceType = random.nextInt(7);  // Random number from 0 to 6
    ...
    public void spawnPiece() {
        currentPiece = Piece.clonePiece(nextPiece); // Clone the next piece
        pieceX = width / 2 - 1;
        pieceY = 0;
        ...
        for (int y = 0; y < currentPiece.shape.length; y++) {
            for (int x = 0; x < currentPiece.shape[y].length; x++) {
                if (currentPiece.shape[y][x] != ' ') {
                    board[pieceY + y][pieceX + x] = currentPiece.color; // Place the piece on the board
                }
            }
        }
    }
        ...
        for (int y = 0; y < piece.length; y++) {
            for (int x = 0; x < piece[y].length; x++) {
                if (piece[y][x] != ' ') {
                    ...
                    // Check if the piece collides with other pieces on the board
    ...
    private void clearLines() {
        for (int y = 0; y < height; y++) {
            ...
            if (fullLine) {
                removeLine(y);
                addScore(100); // Add 100 points for each cleared line
            }
        }
    }
    ...
    public void resetBoard() {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                board[y][x] = Color.BLACK; // Set all cells to black
            }
        }
    }

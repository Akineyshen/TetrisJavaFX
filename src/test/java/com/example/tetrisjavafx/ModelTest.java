...
class ModelTest {
    ...
    @BeforeEach
    void setUp() {
        model = new Model(10, 20);  // Initialize the model with a 10x20 board
    }
    @Test
    void clonePiece() {
        char[][] shape = model.getNextPiece();  // Получаем форму (char[][])
        Color color = model.getNextPieceColor();  // Получаем цвет фигуры
        Model.Piece piece = new Model.Piece(shape, color);  // Создаём объект Piece из формы и цвета

        Model.Piece clonedPiece = Model.clonePiece(piece);  // Клонируем Piece

        // Проверяем, что это разные объекты
        assertNotSame(piece, clonedPiece, "Cloned piece should not be the same object.");

        // Проверяем, что формы у клонированной и исходной фигуры одинаковые
